...
// PERF: memo prevents re-render of every list item when parent state changes (e.g. menu toggle)
const SortableMenuItem = memo(function SortableMenuItem({ item, isActive, onNavigate }) {
    ...
    <div
      ...
      onMouseEnter={() => {
        ...
        if (item.href) {
          ...
          prefetchTimer.current = setTimeout(() => preloadRoute(item.href), 200)
        }
      }}
      onMouseLeave={() => {
        setIsHovered(false)
...
// PERF: memo prevents re-render of every list item when parent state changes
const SortableDocMenuItem = memo(function SortableDocMenuItem({ item, isOpen, isActive, onClick }) {
  const [isHovered, setIsHovered] = useState(false)
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: item.id })
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div
            onClick={onClick}
            style={{ transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s', cursor: 'pointer' }}
          >
            <Icons.ChevronDown />
          </div>
          <div
...
export default memo(function Sidebar() {
  ...
  // PERF: stable toggle callbacks for memo'd SortableDocMenuItem
  const toggleVentas = useCallback(() => setVentasMenuOpen(v => !v), [])
  const togglePorOrganizar = useCallback(() => setPorOrganizarMenuOpen(v => !v), [])
  ...
  // PERF: useCallback so sidebar footer button doesn't cause child re-renders
  const handleSignOut = useCallback(async () => {
    await signOut()
    window.location.href = '/login'
  }, [signOut])

  // PERF: stable callback for SortableMenuItem (memo'd) — avoids re-render on parent state changes
                ...
                {visibleSections.map((item) => {
                  if (item.type === 'submenu') {
                    const isOpen = item.id === 'ventas' ? ventasMenuOpen
                      ...
                      : false
                    // PERF: use stable callbacks instead of inline arrows
                    const toggleOpen = item.id === 'ventas' ? toggleVentas
                      : item.id === 'por-organizar' ? togglePorOrganizar
                      : undefined
                            ...
                            {item.children.map(child => (
                              <Link
                                ...
                                onMouseEnter={() => {
                                  ...
                                  prefetchTimeoutRef.current = setTimeout(() => preloadRoute(child.href), 200)
                                }}
                                onMouseLeave={() => clearTimeout(prefetchTimeoutRef.current)}
                              >
                                <child.icon />
                                <span className="nav-label" style={{ flex: 1 }}>{child.name}</span>
                                {child.href === '/ventas/notificaciones' && <NotificacionesBadge contador={notifContador} />}
