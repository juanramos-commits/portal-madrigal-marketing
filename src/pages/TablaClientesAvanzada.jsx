...
// Colores del tema de la app (tokens.css)
const ESTADOS = {
...
// TODAS las columnas disponibles organizadas por grupo (51 columnas)
...
export default function TablaClientesAvanzada() {
  ...
  const getEstadoBadge = (estado) => {
    const config = ESTADOS[estado] || { label: estado, color: 'var(--text-muted)', bg: 'rgba(255,255,255,0.05)' }
  ...
  const formatValue = (value, col) => {
    if (value === null || value === undefined || value === '') return <span style={{ color: 'var(--text-muted)', opacity: 0.3 }}>—</span>
    if (col.isEstado) return getEstadoBadge(value)
    if (col.isBool) return value ? <span style={{ color: '#2ee59d' }}>✓</span> : <span style={{ color: 'var(--text-muted)', opacity: 0.3 }}>—</span>
    if (col.isDate) {
      try { return new Date(value).toLocaleDateString('es-ES') } catch { return value }
    }
    if (col.isUrl && value) {
      return <a href={value} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-muted)', textDecoration: 'underline', textUnderlineOffset: '2px' }} onClick={(e) => e.stopPropagation()}>{value.replace(/^https?:\/\/(www\.)?/, '').substring(0, 22)}</a>
    }
    return String(value)
  }
    ...
    <div style={{ padding: '24px', minHeight: '100vh' }}>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px', background: 'var(--bg-input)', borderRadius: '8px', border: '1px solid var(--border)' }}>
            <Checkbox checked={nuevoCliente.tiene_whatsapp} onChange={v => setNuevoCliente(prev => ({ ...prev, tiene_whatsapp: v }))} />
            ...
            <span style={{ color: 'var(--text)', cursor: 'pointer' }} onClick={() => setNuevoCliente(prev => ({ ...prev, tiene_whatsapp: !prev.tiene_whatsapp }))}>Tiene WhatsApp</span>
          </div>
        </div>
      </Modal>
