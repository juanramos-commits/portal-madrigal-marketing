-- Fix: handle users with multiple roles (e.g. Mercedes = setter + closer)
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    -- Auto-detect admin from DB (frontend may pass false due to timing)
    ...
    -- Get ALL commercial roles for non-admin users
    IF NOT p_es_admin THEN
      SELECT
        bool_or(rol = 'setter'),
        bool_or(rol = 'closer')
  ...
  FROM (
    ...
    WHERE lp.pipeline_id = p_pipeline_id
      ...
      AND (
        ...
        OR (lower(v_pipeline.nombre) LIKE '%setter%' AND v_is_setter AND l.setter_asignado_id = p_user_id)
        OR (lower(v_pipeline.nombre) LIKE '%closer%' AND v_is_closer AND l.closer_asignado_id = p_user_id)
      )
      -- Filters
