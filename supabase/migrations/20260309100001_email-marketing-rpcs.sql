...
CREATE OR REPLACE FUNCTION em_build_segment_where(
    p_rules JSONB,
...
BEGIN
    ...
    IF p_depth >= p_max_depth THEN
        RAISE EXCEPTION 'em_build_segment_where: profundidad máxima (%) alcanzada', p_max_depth;
    END IF;
    ...
    IF v_operator IS NOT NULL THEN
        ...
        FOR i IN 0 .. jsonb_array_length(v_conditions) - 1 LOOP
            ...
            v_parts := array_append(v_parts, '(' || em_build_segment_where(v_cond, p_depth + 1, p_max_depth) || ')');
        END LOOP;
    ...
    -- Determinar tabla según campo (con cast seguro para numéricos)
    CASE v_field
        ...
        WHEN 'engagement_score',
             ...
             'total_opened',
             'total_clicked'    THEN
            -- Cast seguro a INT: si falla, devolver FALSE en vez de error
            BEGIN
                v_int_value := v_value::INT;
            EXCEPTION WHEN OTHERS THEN
                RAISE WARNING 'em_build_segment_where: valor "%" no es numérico para campo "%"', v_value, v_field;
                RETURN 'FALSE';
            END;
            v_clause := format('c.%I %s %s', v_field, v_sql_op, v_int_value);
...
BEGIN
    ...
    -- Actualizar conteo y timestamp usando la misma cláusula WHERE dinámica
    EXECUTE format(
        'UPDATE ventas_em_segments SET contact_count = ('
        '  SELECT count(*) FROM ventas_em_contacts c'
        '  LEFT JOIN ventas_leads l ON l.id = c.lead_id'
        '  WHERE c.status = ''active'' AND (%s)'
