-- =====================================================
-- CRM: Añadir filtro por etiquetas a obtener_crm_completo
-- =====================================================
...
CREATE OR REPLACE FUNCTION obtener_crm_completo(
  ...
  p_filtro_etiqueta_ids UUID[] DEFAULT NULL,
...
BEGIN
  ...
  FROM (
    ...
    WHERE lp.pipeline_id = p_pipeline_id
      ...
      AND (p_filtro_etiqueta_ids IS NULL OR EXISTS (
        SELECT 1 FROM ventas_lead_etiquetas le
        WHERE le.lead_id = l.id AND le.etiqueta_id = ANY(p_filtro_etiqueta_ids)
      ))
